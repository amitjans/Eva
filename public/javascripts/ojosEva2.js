...
function init(){
  ...
  //scene.add( new THREE.AmbientLight( 0xffffff) );
	...
	//camera.add( light );
    addLights();
    //agregarOjos("ojo7.json");
  //agregarParpados("parpadoBueno2.json");
  //agregarEscena("ojo7.json","parpadoBueno2.json");
  //agregarFixPoint();
    agregarCorazon();

    //cargarOjo();
    //var selectedObject = scene.getObjectByName("parpadoTop");
    /*setTimeout(function(){
      eliminarEscena();
      agregarFixPoint();
      animate();
    ...
    setTimeout(function(){
      scene.remove(torus);
      agregarEscena("ojo_verde.json","parpadoBuenoVerde.json");
      animate();
    },15000);*/
...
function agregarEscena(nombreOjo,nombreParpado){
  agregarOjos(nombreOjo);
  setTimeout(function(){
      agregarParpados(nombreParpado);
  },800);
...
var animacionFix, animacionFix2, animacionFix3;
function tweenFix(){
  var value = Math.PI;
  animacionFix = new TWEEN.Tween(torus.rotation)
                      .to({x:-value,y:-value,z:-value},2000)
                      .delay(0)
                      .easing(TWEEN.Easing.Linear.None)
                      .onUpdate(function(){
                          torus.rotation.x=torus.rotation.x;
                          torus.rotation.y=torus.rotation.y;
                          torus.rotation.z=torus.rotation.z;
                      });
  animacionFix2 = new TWEEN.Tween(torus.rotation)
                      .to({x:value,y:value,z:value},2000)
                      .delay(0)
                      .easing(TWEEN.Easing.Linear.None)
                      .onUpdate(function(){
  ...
  animacionFix.chain(animacionFix2);
  animacionFix2.chain(animacionFix);
  animacionFix.start();
};
...
var parpado = null;
var tamEye = 1.0;
function agregarParpados(nombreArchivo) {
    var loader = new THREE.JSONLoader();

    loader.load('/javascripts/'+nombreArchivo, function(geometry, materials) {
        parpadoTop = new THREE.Mesh(geometry, new THREE.MeshFaceMaterial(materials));
        parpadoTop.name = "parpadoTop";
        parpadoTop.scale.x = parpadoTop.scale.y = parpadoTop.scale.z = 1.01;
...
function agregarOjos(nombreArchivo){
    var loader = new THREE.JSONLoader();
...
function agregarFixPoint(){
  var loader = new THREE.JSONLoader();
  //var loader = new THREE.GLTFLoader();
  loader.load('/javascripts/torus.json', function(geometry, materials) {
      torus = new THREE.Mesh(geometry, new THREE.MeshFaceMaterial(materials));
      torus.scale.x = torus.scale.y = torus.scale.z = 0.2;
      //ojoLeft.position.x = -0.75;
      torus.rotation.x = Math.PI/2;
      torus.translation = THREE.GeometryUtils.center(geometry);
      scene.add(torus);
      tweenFix();
      //animacionNormal();
  });
...
function eliminarEscena(){
  ...
  scene.remove( parpadoBottom );
  scene.remove( parpadoBottom2 );
  scene.remove(ojoLeft);
  scene.remove(ojoRight);
  //animate();
}

var socket = io.connect('192.168.1.76:3000', { 'forceNew': true });
//var socket = io.connect('192.168.1.107:8080', { 'forceNew': true });
//var socket = io.connect('192.168.43.114:8080', { 'forceNew': true });
socket.on('messages', function(data) {
    ...
    switch(action){
        ...
        case 10:
          scene.remove(torus);
          TWEEN.removeAll();
          agregarEscena("ojo_azul.json","parpadoBuenoAzul.json");
          //animate();
        break;
        case 11:
          scene.remove(torus);
          TWEEN.removeAll();
          agregarEscena("ojo_morado.json","parpadoBuenoMorado.json");
          //animate();
        break;
        case 12:
          scene.remove(torus);
          TWEEN.removeAll();
          agregarEscena("ojo_lila.json","parpadoBuenoLila.json");
          //animate();
        break;
        case 13:
          scene.remove(torus);
          TWEEN.removeAll();
          agregarEscena("ojo_amarillo.json","parpadoBuenoAmarillo.json");
          //animate();
        break;
        case 14:
          scene.remove(torus);
          TWEEN.removeAll();
          agregarEscena("ojo_naranja.json","parpadoBuenoNaranja.json");
          //animate();
        break;
        case 15:
          scene.remove(torus);
          TWEEN.removeAll();
          agregarEscena("ojo_rosado.json","parpadoBuenoRosado.json");
          //animate();
        break;
        default:
        break;
