 ...
 * @param {Object} configuration Configuration parameters (see TJBot.prototype.configurationParameters).
...
function TJBot(hardware, configuration, credentials) {
    ...
    this.configuration = {};
    TJBot.prototype.configurationParameters.forEach(function(param) {
        if (configuration != undefined && configuration[param] != undefined) {
            this.configuration[param] = configuration[param];
        } else {
            this.configuration[param] = TJBot.prototype.defaultConfiguration[param];
        }
    }, this);
    ...
    hardware.forEach(function(device) {
        switch (device) {
            ...
            case 'led':
                this._setupLED(this.configuration.ledPin);
                break;
            ...
            case 'servo':
                this._setupServo(this.configuration.servoPin);
                break;
    ...
    // > conversation
    if (credentials != undefined && credentials.hasOwnProperty('conversation')) {
        var creds = credentials['conversation'];
        this._createServiceAPI('conversation', creds);
    }

    // > speech to text
    if (credentials != undefined && credentials.hasOwnProperty('speech_to_text')) {
        var creds = credentials['speech_to_text'];
        this._createServiceAPI('speech_to_text', creds);
    }

    // > text to speech
    if (credentials != undefined && credentials.hasOwnProperty('text_to_speech')) {
        var creds = credentials['text_to_speech'];
        this._createServiceAPI('text_to_speech', creds);
    }

    // > tone analyzer
    if (credentials != undefined && credentials.hasOwnProperty('tone_analyzer')) {
        var creds = credentials['tone_analyzer'];
        this._createServiceAPI('tone_analyzer', creds);
    }

    // > visual recognition
    if (credentials != undefined && credentials.hasOwnProperty('visual_recognition')) {
...
TJBot.prototype._createServiceAPI = function(service, credentials) {
    if (this.configuration.verboseLogging) {
        console.info("> Eva initializing " + service + " service");
    }
    ...
    switch (service) {
        ...
        case 'speech_to_text':
            ...
            /*this._speechGoogle = Speech({
              projectId: 'roboteva-162823',
              keyFilename: './robotEva-2d2c75c12057.json'
            });*/
            break;
...
TJBot.prototype._setupMicrophone = function() {
    if (this.configuration.verboseLogging) {
        console.info("> Eva initializing microphone");
    }
    ...
    // ignore silence
    this._micInputStream.on('silence', function() {console.log('silence.....')});


    // TJ is listening!
    console.log("Eva is listening, you may speak now.");
}
...
TJBot.prototype._sendAudioGoogleSpeechtoText2 = function(callback){
...
TJBot.prototype._sendAudioGoogleSpeechtoText = function(callback){
    if (this.configuration.verboseLogging) console.log("> opening audiostream with google speech to text")

    var self = this;

    //console.log('entro a _sendAudioGoogle');
    var request = {
      config: {
        encoding: 'LINEAR16',
        sampleRate: 16000,
    ...
    var recognizeStream = this._speechGoogle.createRecognizeStream(request);
    /*.on('error', console.error)
    .on('data', function(data){
    ...
    this.micTextStream = this._micInputStream.pipe(recognizeStream);

    this.micTextStream.on('error', function(error){
        console.error("The speech_to_text service returned an error.");
        console.error("Raw error:");
        console.error(err);

        self._mic.resume(); //if paused
        if (self.configuration.ttsReconnect) {
            console.error("=====================\nAttempting to reconnect in 2 seconds");
            self._sendAudioGoogleSpeechtoText(callback);
        }

    });

    this.micTextStream.on('data', function(data){
        //if(data.results){console.log(data); callback(data.results)}
        /*if (callback != undefined && data.results) {
            callback(data.results);
        }*/
        console.log(data);
        if(self.configuration.ttsReconnect && data.error){
            self._mic.resume(); //if paused
            console.error("=====================\nAttempting to reconnect in 2 seconds");
            errorGoogle = true;
            self._sendAudioGoogleSpeechtoText(callback);
        ...
        else if(callback != undefined && data.results){
            console.log("TJ heard: " + data.results);
            callback(data.results);
        }
    });
    ...
    if(!errorGoogle)
        this._mic.start();
    /*var self = this;
    this._micRecognizeStream = speechClient.createRecognizeStream(request);
    ...
    this._micTextStream.on('data', function(transcriptMessage) {
        ...
        console.log("TJ heard: ");
        console.log(JSON.stringify(transcriptMessage));
...
TJBot.prototype._sendAudioWatsonSpeechtoText = function(callback) {
    ...
    if (this.configuration.verboseLogging) console.log("> opening audiostream with watson speech to text")
    // capture context
    var self = this;
    ...
    this._micRecognizeStream = this._stt.createRecognizeStream({
        content_type: 'audio/l16; rate=16000; channels=1',model: 'es-ES_BroadbandModel'
    });
    ...
    // pass any data captured by STT to the callback
    this._micTextStream.on('data', function(transcriptMessage) {
        console.log("Eva heard: " + transcriptMessage);
...
TJBot.prototype._assertCapability = function(capability) {
    switch (capability) {
        case 'listen':
            ...
            /*if (!this._mic || !this._speechGoogle) {
                throw new Error('TJBot is not configured to listen Google. Please check you included the correct hardware capabilities in the TJBot constructor.');
            }*/
            break;
...
TJBot.prototype.converse = function(workspaceId, message, callback) {
    ...
    context.timezone='America/Tijuana';
    //console.log(context);
...
TJBot.prototype.callVisualRecognition = function(mode, filePath) {
    ...
    return new Promise(function(resolve, reject) {
        var visionParams = {
            ...
            threshold: self.configuration.visionConfidenceThreshold
        };
        if (mode == "classify" || mode === undefined) {
            self._visualRecognition.classify(visionParams, function(err, visionResponseObject) {
                if (err) {
                    ...
                    if (Array.isArray(visionClassifierResult)) {
                        ...
                        if (self.configuration.verboseLogging) console.log(imageDescription)
                        var response = {
                            "object": visionResponseObject,
                            "description": imageDescription
                        };
            ...
            self._visualRecognition.recognizeText(visionParams, function(err, visionResponseObject) {
                visionParams = {
                    images_file: fs.createReadStream(filePath),
                    threshold: self.configuration.visionTextConfidenceThreshold
                };
                if (err) {
                    console.log(err);
                    reject(err)
                } else {
                    if (self.configuration.verboseLogging) console.log(visionResponseObject.images[0].text)
                    var response = {
                        "object": visionResponseObject,
...
TJBot.prototype.captureImage = function(filePath) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        self.configuration.cameraParams.height ? cameraParams.push('-h', self.configuration.cameraParams.height) : cameraParams.push('-h', self.defaultConfiguration.cameraParams.height);
        self.configuration.cameraParams.width ? cameraParams.push('-w', self.configuration.cameraParams.width) : cameraParams.push('-w', self.defaultConfiguration.cameraParams.width);
        if (self.configuration.cameraParams.vflip) cameraParams.push('-vf');
        if (self.configuration.cameraParams.hflip) cameraParams.push('-hf');
 ...
 */
 ...
 TJBot.prototype.speakNew = function(message,cb) {
             ...
             self._tts.synthesize(utterance)
                 ...
                 .on('close', function() {
                     if (self.configuration.verboseLogging) {
                         ...
                         console.info("> Eva says: " + message);
                         //cb(message);
                     }
                     resolve(self.playSound(info.path,4,cb,message));
                     //resolve(self.playSound('./sonidos/laugh-03.wav',1));
                 });
         });
     });
 }

TJBot.prototype.speak = function(message,tipo,cb1,cb2) {
    this._assertCapability('speak');
        ...
        temp.open('tjbot', function(err, info) {
            ...
            self._tts.synthesize(utterance)
                ...
                .on('close', function() {
                    ...
                    resolve(self.playSound(info.path,tipo,cb1,message,cb2));
                    //resolve(self.playSound('./sonidos/laugh-03.wav',1));
                });
        });
    });
...
TJBot.prototype.playSound = function(soundFile,tipo,cb,msg,cb2) {
    ...
    if (!self._isPlaying) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            player.on('complete', function() {
                ...
                if(tipo)
                    switch(tipo){
                        case 1: resolve(self.playSound('./sonidos/laugh-03.wav'));
                        break;
                        case 2: resolve(self.playSound('./sonidos/applause2_x.wav'));
                        break;

                        case 3: resolve(self.playSound('./sonidos/fanfare2.wav',5,cb,msg,cb2));

                        break;
                        case 4:

                        //resolve(self.playSound('./sonidos/beep-02.wav'));
                        break;
                        case 5:
                        //if(cancion)
                          //  setTimeout(function(){resolve(self.playMusic(cancion))},500);
                        break;
                        case 6:
                            setTimeout(function(){resolve(self.playMusic('./preview.wav'))},500);
                        break;
                        case 7:
                            resolve(self.playSound('./sonidos/cash_register_x.wav'));
                        default: break;
                    }


                    //self.speak('./sonidos/laugh-03.wav');
            });
        ...
        if (self.configuration.verboseLogging) console.log("> Speaker in use, try playing audio later.");
    };
...
/*youTube.search('amor de mis amores agustín lara', 2, function(error, result) {
  ...
  if (error) {
    ...
    var requestUrl = 'https://www.youtube.com/watch?v='+ result.items[0].id.videoId;
    v.setVolume(0.15);
...
TJBot.prototype.playMusic = function(soundFile){
...
TJBot.prototype.stopMusic = function(){
    var self = this;
    console.info('stopMusic');
    if(music)
        music.stop();
}

TJBot.prototype.pauseMusic = function(){
    var self = this;
    console.info('pauseMusic');
    //console.log(music);
    if(music){
        music.pause();
        self._isPlaying = false;
...
TJBot.prototype.resumeMusic = function(){
    var self = this;

    console.info('resumeMusic');
    //console.log(music);
    if(music){
        self._isPlaying = true;
        self.pauseListening();
...
TJBot.prototype.listen = function(callback) {
    ...
    this._sendAudioGoogleSpeechtoText2(callback);
}
